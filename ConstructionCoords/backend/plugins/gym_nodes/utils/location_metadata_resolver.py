...
class LocationMetadataResolver:
    ...
    def annotate_location(self, gym_node) -> None:
        reverse_geocode_response: str | None = self.reverse_geocode(gym_node)
        print(f"--| OpenStreetMap response for [lat={gym_node.latitude}, lon={gym_node.longitude}]: \n'''\n{reverse_geocode_response}\n'''")

        prompt: str = self.construct_prompt(gym_node.latitude, gym_node.longitude, reverse_geocode_response)
        ...
        if open_ai_response is not None:
            try:
                ...
                print(f"--| OpenAI response for [lat={gym_node.latitude}, lon={gym_node.longitude}]:\n'''\n{json.dumps(parsed_open_ai_response, indent=2, ensure_ascii=False)}\n'''")
    ...
    def reverse_geocode(self, gym_node) -> str | None:
        try:
            location = self._geolocator.reverse(
                (gym_node.latitude, gym_node.longitude),
                language="en",
                timeout=5
            )
